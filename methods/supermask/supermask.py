...
class SuperMask(EnsembleMethod):
    ...
    def train(self, epochs, train_dataset, eval_dataset, test_dataset, optimizer, scheduler=None,
        ...
        #
        # masks_parameters = [param for name, param in self.model.named_parameters() if 'distributions' in name]
        #
...
class BatchSuperMask(EnsembleMethod):
    ...
    def train(self, epochs, train_dataset, eval_dataset, test_dataset, optimizer, scheduler=None,
              regularization=None, early_stopping=None,
              **kwargs):
        layer_to_masked(self.model, masks_params=self.supermask_parameters, ensemble=self.ensemble, batch_ensamble=True)
