...
class BatchForwardPruningSuperMask(EnsembleMethod):
    def __init__(self, model, method_parameters,
                 device='cpu', **kwargs):
        super().__init__(**kwargs)
    ...
    def train_models(self, epochs, train_dataset, eval_dataset, test_dataset, optimizer, scheduler=None,
        ...
        add_wrappers_to_model(self.model,
                              ...
                              batch_ensemble=True,
                              training_supermask=True,
                              pruning_percentage=self.prune_percentage)
        print(self.model)
        ...
        for name, module in self.model.named_modules():
            if isinstance(module, BatchEnsembleForwardMaskedWrapper):
                m = module.last_mask
        ...
        for mi, _masks in masks.items():

            model = extract_inner_model(self.model, _masks, re_init=self.re_init)
