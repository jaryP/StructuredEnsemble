...
from torch.distributions import Beta, Normal, Uniform, Laplace


def gaussian_kernel(a, b, gamma=2.0):
    a = a.squeeze()
    b = b.squeeze()
...
def MMD(posterior, prior):
    # assert isinstance(posterior, TrainableMask)

    a = posterior()
    b = prior()

    # if hasattr(prior, 'sample'):
    #     b = prior.sample(a.size())
    # else:
    #     b = prior
    #
...
def get_distribution(name, **kwargs):
    name = name.lower()
    if name == 'normal':
        distribution = Normal(kwargs['mean'], kwargs['std'])
    elif name == 'uniform':
...
class TrainableNormal(TrainableMask):
    def __init__(self, dimensions, initialization, t=1):
        ...
        self.std = get_trainable_mask(dimensions, initialization['sigma'])
    ...
    def __repr__(self):
        return '{}(mu={}, sigma={})'.format(self.__class__.__name__, tuple(self.mu.shape), tuple(self.std.shape))
...
# class TrainableBernulli(nn.Module):
#     def __init__(self, dimesions, p):
#         super().__init__()
#         self.p = get_bernoulli_parameters(dimesions, p)
#
#     @property
#     def posterior(self):
#         return Bernoulli(torch.sigmoid(self.p))